...
atrtrailer = nil
-- dgun combo
local item_list = {'Кулак', 'Кастет', 'Клюшка для гольфа', 'Полицейская дубинка', 'Нож', 'Бейсбольная бита', 'Лопата', 'Кий', 'Катана', 'Бензопила', 'Двухсторонний дилдо', 'Дилдо', 'Вибратор', 'Серебряный вибратор', 'Букет цветов', 'Трость', 'Граната', 'Слезоточивый газ', 'Коктейль Молотова', ' ', ' ', ' ', 'Пистолет', 'USP-S (Тазер)', 'Desert Eagle', 'Дробовик', 'Обрез', 'Скорострельный дробовик', 'UZI', 'MP5', 'Калаш', 'М4', 'TEC-9', 'Охотничье ружье', 'Снайперка', 'РПГ', 'Самонаводящееся РПГ', 'Огнемет', 'Миниган', 'Сумка с тротилом', 'Детонатор', 'Баллончик', 'Огнетушитель', 'Фотоаппарат', 'Очки ночного видения', 'Тепловизор', 'Парашют'}
...
imgui.OnFrame(function() return found_update[0] end, function()
    ...
    imgui.Begin('', found_update, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysAutoResize)
    ...
    imgui.CenterText('')
    imgui.PushFont(upd1)
    imgui.CenterText('New update found!')
    imgui.PopFont()
    imgui.CenterText('')
    imgui.CenterText('')
    imgui.PushFont(upd2)
    imgui.CenterText('A new script update was found')
    imgui.CenterText('To continue working you')
    imgui.CenterText('must select one of the two buttons below')
    imgui.PopFont()
    imgui.CenterText('')
    imgui.CenterText('')
    if imgui.Button(fa.DOWNLOAD..' ОБНОВИТЬ', imgui.ImVec2(650, 40)) then
        updateScript(lmUrl, lmPath)
    end
    if imgui.Button(fa.FORWARD..' ПРОПУСТИТЬ', imgui.ImVec2(650, 40)) then
       found_update[0] = not found_update[0]
       notf4('Обновление скрипта пропущено')
    end
    imgui.End()
end)
-- demo window
...
imgui.OnFrame(function() return settings.menu.sendalt[0] end, function()
    ...
    imgui.Begin('  ', settings.menu.sendalt, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
    if imgui.Button(fa.HANDSHAKE_ANGLE..' Взаимодействие', imgui.ImVec2(settings.menu.sendaltwidth[0], settings.menu.sendaltheight[0])) then
        lua_thread.create(function()
            local bs = raknetNewBitStream()
...
imgui.OnFrame(function() return menusettings[0] end, function()
    imgui.Begin('Настройки меню', menusettings, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
    imgui.CenterText(fa.FOLDER..' Общие настройки меню')
    imgui.Separator()
    if imgui.SliderFloat(fa.MAXIMIZE..' Размер шрифта', settings.menu.window_scale, 1 / MONET_DPI_SCALE, 3.0) then
    ...
    imgui.Separator()
    imgui.CenterText(fa.HANDSHAKE_ANGLE..' Взаимодействие')
    imgui.Separator()
    if imgui.SliderInt(fa.TEXT_WIDTH..' Ширина взаимодействия', settings.menu.sendaltwidth, 1, 400) then
        if settings.cfg.autosave[0] then
            ini.menu.sendaltwidth = settings.menu.sendaltwidth[0]
            save()
        end
    end
    if imgui.SliderInt(fa.TEXT_HEIGHT..' Высота взаимодействия', settings.menu.sendaltheight, 1, 400) then
        if settings.cfg.autosave[0] then
...
imgui.OnFrame(function() return window_state[0] end, function()
    ...
    imgui.Begin("##miku", window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoScrollbar)
    ...
    if imgui.BeginChild("Tabs##"..tab, imgui.ImVec2(153 * MONET_DPI_SCALE, 405 * MONET_DPI_SCALE), false) then
        if imgui.Button(fa.FIRE.."  Основное", imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 1
            activetab = 1
        end
        if activetab == 1 then
        ...
        if imgui.Button(fa.USER..'  Персонаж', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 2
            activetab = 2
        end
        if activetab == 2 then
        ...
        if imgui.Button(fa.CAR..'  Машина', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 3
            activetab = 3
        end
        if activetab == 3 then
        ...
        if imgui.Button(fa.EYE..'  Подсветка', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 4
            activetab = 4
        end
        if activetab == 4 then
        ...
        if imgui.Button(fa.GAVEL..'  ТСР Хелпер', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 5
            activetab = 5
        end
        if activetab == 5 then
        ...
        if imgui.Button(fa.SQUARE_CHECK..'  Кнопочки', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 6
            activetab = 6
        end
        if activetab == 6 then
        ...
        if imgui.Button(fa.GEARS..'  Настройки', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 7
            activetab = 7
        end
        if activetab == 7 then
    ...
    if imgui.BeginChild("##MenuT", imgui.ImVec2(652 * MONET_DPI_SCALE, 458 * MONET_DPI_SCALE), false) then
        if tab == 1 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button("Основное") then
                subtab_1 = 1
            end
            imgui.SameLine()
            if imgui.Button("Рендер") then
                subtab_1 = 2
            end
            imgui.SameLine()
            if imgui.Button("Объекты") then
                subtab_1 = 3
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_1 == 1 then
                imgui.Text('Аирбрейк активируется даблтапом на иконку оружия')
                if imgui.ToggleButton('Виджет аирбрейка', settings.main.airbrakewidget) then
                    if settings.cfg.autosave[0] then
                        ini.main.airbrakewidget = settings.main.airbrakewidget[0]
                        save()
                    end
                end
                if imgui.ToggleButton('ClickCoordMobile', settings.main.clickwarpcoord) then
                    if settings.cfg.autosave[0] then
                        ini.main.clickwarpcoord = settings.main.clickwarpcoord[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Анти-маска', settings.main.antimask) then
                    if settings.cfg.autosave[0] then
                        ini.main.antimask = settings.main.antimask[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Анти-капча', settings.main.autocaptcha) then
                    if settings.cfg.autosave[0] then
                        ini.main.autocaptcha = settings.main.autocaptcha[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Напоминание о низком заряде', settings.battery.notifyLowCharge) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.battery.notifyLowCharge[0] then
                    ...
                    if imgui.SliderInt('Уровень заряда', settings.battery.lowBatteryLevel, 1, 100) then
                ...
                if imgui.ToggleButton("Антифриз", settings.main.antiFreeze) then
                    if settings.cfg.autosave[0] then
                        ini.main.antiFreeze = settings.main.antiFreeze[0]
                        save()
                    end
                end
                if imgui.Button('Реконнект', imgui.ImVec2(80 * MONET_DPI_SCALE, 30 * MONET_DPI_SCALE)) and not Reconnect.reconnecting and not Reconnect.waiting then
                    Reconnect.activate()
                end
                imgui.SameLine()
                if Reconnect.waiting then
                    imgui.SameLine()
                    imgui.Text(string.format('Реконнект через %.2f секунд...', Reconnect.remaining / 1000))
                    imgui.SameLine()
                    if imgui.Button(fa.XMARK..' Отмена') then
                        Reconnect.abort = true
                    end
                end
                imgui.SetNextItemWidth(imgui.GetFontSize() * 15)
                imgui.SliderFloat('Задержка реконнекта (в секундах)', Reconnect.delay, 0.0, 30.0)
                if imgui.Button('Установить погоду') then
                    forceWeatherNow(WeatherAndTime.weather[0])
                end
                imgui.SameLine()
                imgui.SetNextItemWidth(imgui.GetFontSize() * 5)
                imgui.PushItemWidth(120)
                if imgui.InputInt('Погода', WeatherAndTime.weather, 1, 10) then
                ...
                imgui.PopItemWidth()
                imgui.SameLine()
                if imgui.ToggleButton('Блокировать погоду сервера', settings.main.weatherblockserv) then
                    if settings.cfg.autosave[0] then
                        ini.main.weatherblockserv = settings.main.weatherblockserv[0]
                        save()
                    end
                end
                if imgui.Button('Установить время') then
                ...
                imgui.SetNextItemWidth(imgui.GetFontSize() * 5)
                imgui.PushItemWidth(120)
                if imgui.InputInt('Время', WeatherAndTime.time, 1, 5) then
                ...
                imgui.PopItemWidth()
                imgui.SameLine()
                if imgui.ToggleButton('Блокировать время сервера', settings.main.timeblockserv) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_1 == 2 then
                if imgui.ToggleButton('Руда', settings.render.ruda) then
                    if settings.cfg.autosave[0] then
                        ini.render.ruda = settings.render.ruda[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Олени', settings.render.deer) then
                    if settings.cfg.autosave[0] then
                        ini.render.deer = settings.render.deer[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Семена нарко', settings.render.semena) then
                    if settings.cfg.autosave[0] then
                        ini.render.semena = settings.render.semena[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Закладки', settings.render.narkotiki) then
                    if settings.cfg.autosave[0] then
                        ini.render.narkotiki = settings.render.narkotiki[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Дерево высшего качества', settings.render.derevovishkac) then
                    if settings.cfg.autosave[0] then
                        ini.render.derevovishkac = settings.render.derevovishkac[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Подарок', settings.render.podarok) then
                    if settings.cfg.autosave[0] then
                        ini.render.podarok = settings.render.podarok[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Клад', settings.render.kladrender) then
                    if settings.cfg.autosave[0] then
                        ini.render.kladrender = settings.render.kladrender[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Яблочное дерево', settings.render.yabloki) then
                    if settings.cfg.autosave[0] then
                        ini.render.yabloki = settings.render.yabloki[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Cливовое дерево', settings.render.slivu) then
                    if settings.cfg.autosave[0] then
                        ini.render.slivu = settings.render.slivu[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Кокосное дерево', settings.render.kokosi) then
                    if settings.cfg.autosave[0] then
                        ini.render.kokosi = settings.render.kokosi[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Граффити', settings.render.graffiti) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_1 == 3 then
                imgui.Text('Удалить:')
                if imgui.Button('Ворота армии ЛС', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Автоудаление', settings.objects.autormlsa) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormlsa = settings.objects.autormlsa[0]
                        save()
                    end
                end
                if imgui.Button('Ворота армии СФ', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 988 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Автоудaление', settings.objects.autormsfa) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormsfa = settings.objects.autormsfa[0]
                        save()
                    end
                end
                if imgui.Button('Шлагбаумы (КПП)', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 968 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Aвтоудаление', settings.objects.autormblockpost) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormblockpost = settings.objects.autormblockpost[0]
                        save()
                    end
                end
                if imgui.Button('Фонарные столбы', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 1297 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Автoудаление', settings.objects.autormlampposts) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormlampposts = settings.objects.autormlampposts[0]
                        save()
                    end
                end
                if imgui.Button('Дорожные ремонты', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 1422 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Автoудаление##roadrem', settings.objects.autormroadrem) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.menu.showinfo[0] then
                    imgui.Text('| (Будет пополняться в зависимости)')
                    imgui.Text('| (от ваших пожеланий в чате Miku Project)')
                end
            end
        elseif tab == 2 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.16, 0.16, 0.18, 1.00))
            ...
            imgui.SameLine()
            if imgui.Button("Silent Aim##tab") then
                subtab_2 = 2
            end
            imgui.SameLine()
            if imgui.Button("Оружие") then
                subtab_2 = 3
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_2 == 1 then
                if imgui.ToggleButton('Бессмертие', settings.ped.godmode_enabled) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton('Ваншот ботов', settings.ped.killbots1hit) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton('AutoScroll', settings.ped.autoscroll) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##scrollsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup('Настройки автоскролла')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal('Настройки автоскролла', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt('Патроны##as', settings.ped.pt, 1, 100) then
                        if settings.cfg.autosave[0] then
                            ini.ped.pt = settings.ped.pt[0]
                            save()
                        end
                    end
                    if imgui.SliderInt('Задержка скролла', settings.ped.wait, 1, 1000) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton('No Reload', settings.ped.noreload) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton('Фейк скиллы', settings.ped.setskills) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(400 * MONET_DPI_SCALE)
                if imgui.ToggleButton('Анти падение', settings.ped.nofall) then
                    if settings.cfg.autosave[0] then
                        ini.ped.nofall = settings.ped.nofall[0]
                        save()
                    end
                end
                imgui.ToggleButton('Флуд взаимодействием', floodalt)
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton('Авто +C', settings.ped.autoplusc) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(400 * MONET_DPI_SCALE)
                if imgui.ToggleButton('Поле зрения', settings.ped.changefov) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##fovsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup('Настройки поля зрения')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal('Настройки поля зрения', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt("Значение FOV'a", settings.ped.fovvalue, 50, 130) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton('Виджет сбива', settings.ped.sbiv) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton('Бесконечный бег', settings.ped.infiniterun) then
                    if settings.cfg.autosave[0] then
                        ini.ped.infiniterun = settings.ped.infiniterun[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Скорость анимаций', settings.ped.animspeed) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##speedsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup('Настройки скорости анимаций')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal('Настройки скорости анимаций', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt('Множитель скорости##animspeed', settings.ped.speedint, 1, 10) then
                        if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton('Рапид', settings.ped.rapidfire) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##rapidsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup('Настройки рапида')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal('Настройки рапида', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt('Множитель рапида##rapidfire', settings.ped.rapidint, 1, 10) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.Button('Установить скин', imgui.ImVec2(300 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    setskin_activate()
                end
                imgui.SameLine()
                imgui.SetNextItemWidth(imgui.GetFontSize() * 7)
                if imgui.InputInt('ID Скина', settings.ped.skinid, 1, 50) then
                    if settings.cfg.autosave[0] then
                        ini.ped.skinid = settings.ped.skinid[0]
                        save()
                    end
                end
                if imgui.Button('Включить бег CJ', imgui.ImVec2(146 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    setAnimGroupForChar(PLAYER_PED, "PLAYER")
                    notf4('Бег CJ включен!')
                end
                imgui.SameLine()
                if imgui.Button('Выключить бег CJ', imgui.ImVec2(146 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    setAnimGroupForChar(PLAYER_PED, usePlayerAnimGroup and "PLAYER" or isCharMale(PLAYER_PED) and "MAN" or "WOMAN")
                    notf4('Бег CJ выключен!')
                end
                imgui.PopItemWidth()
            elseif subtab_2 == 2 then
                if imgui.ToggleButton('Silent Aim', settings.silent.salo) then
                    if settings.cfg.autosave[0] then
                        ini.silent.salo = settings.silent.salo[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Игнорировать автомобили', settings.silent.ignoreCars) then
                    if settings.cfg.autosave[0] then
                        ini.silent.ignoreCars = settings.silent.ignoreCars[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Игнорировать объекты', settings.silent.ignoreObj) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.Separator()
                if imgui.ToggleButton('Автоматически выбирать дистанцию у оружие', settings.silent.useWeaponDistance) then
                    if settings.cfg.autosave[0] then
                ...
                if not settings.silent.useWeaponDistance[0] then
                    if imgui.SliderInt('Дистанция поиска цели', settings.silent.distance, 1, 1000) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.SliderInt('Радиус поиска цели', settings.silent.radius, 1, 100) then
                    if settings.cfg.autosave[0] then
                        ini.silent.radius = settings.silent.radius[0]
                        save()
                    end
                end
                if imgui.SliderFloat('Положение FOV по X', settings.silent.offsetx, -200.0, 200.0) then
                    if settings.cfg.autosave[0] then
                        ini.silent.offsetx = settings.silent.offsetx[0]
                        save()
                    end
                end
                if imgui.SliderFloat('Положение FOV по Y', settings.silent.offsety, -200.0, 200.0) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.Separator()
                if imgui.ToggleButton('Отображать линию к цели', settings.silent.line) then
                    if settings.cfg.autosave[0] then
                        ini.silent.line = settings.silent.line[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Отображать прицел', settings.silent.circle) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.silent.circle[0] then
                    if imgui.ToggleButton('Отображать более производительный прицел (более удобный)', settings.silent.fpscircle) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton('Писать снизу о нанесение урона', settings.silent.printString) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.Separator()
                if imgui.ToggleButton('Промахи', settings.silent.misses) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.silent.misses[0] then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt('Шансы на промах', settings.silent.miss_ratio, 1, 100) then
                        if settings.cfg.autosave[0] then
                ...
                imgui.ToggleButton('Изменение камеры', fakemode)
            elseif subtab_2 == 3 then
                if imgui.Button('Выдать ган') then
                    givePlayerGun(settings.dgun.gunsList[0], settings.dgun.ammo[0])
                end
                imgui.PushItemWidth(350)
                if imgui.Combo('Ганы',settings.dgun.gunsList,ImItems, #item_list) then
                    ini.dgun.gunsList = settings.dgun.gunsList[0]
                    save()
                end
                imgui.PopItemWidth()
                imgui.PushItemWidth(350)
                if imgui.SliderInt('Патроны##dgun', settings.dgun.ammo, 1, 2000) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.PopItemWidth()
                if settings.menu.showinfo[0] then
                    imgui.Text("| Кикает без Resync'a, потому подойдет только для нуборп")
                end
            end
        elseif tab == 3 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.16, 0.16, 0.18, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button("Машина") then
                subtab_3 = 1
            end
            imgui.SameLine()
            if imgui.Button("Attach Trailer##atr") then
                subtab_3 = 2
            end
            imgui.SameLine()
            if imgui.Button("SpeedHack") then
                subtab_3 = 3
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_3 == 1 then
                if imgui.ToggleButton('Бессмертие', settings.car.godmode2_enabled) then
                    if settings.cfg.autosave[0] then
                        ini.car.godmode2_enabled = settings.car.godmode2_enabled[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Полёт', settings.car.flycar) then
                    if settings.cfg.autosave[0] then
                        ini.car.flycar = settings.car.flycar[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Анти-падение с Мото', settings.car.nobike) then
                    if settings.cfg.autosave[0] then
                        ini.car.nobike = settings.car.nobike[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Бесконечное топливо', settings.car.infinitefuel) then
                    if settings.cfg.autosave[0] then
                        ini.car.infinitefuel = settings.car.infinitefuel[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Быстрая посадка в авто', settings.car.fastenter) then
                    if settings.cfg.autosave[0] then
                        ini.car.fastenter = settings.car.fastenter[0]
                        save()
                    end
                end
	            if imgui.ToggleButton('Быстрый выход', settings.car.fastexit) then
                    if settings.cfg.autosave[0] then
                        ini.car.fastexit = settings.car.fastexit[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Быстрый тормоз + флип', settings.car.fastbrake) then
                    if settings.cfg.autosave[0] then
                        ini.car.fastbrake = settings.car.fastbrake[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Прыжок авто (start)', settings.car.jumpcar) then
                    if settings.cfg.autosave[0] then
                        ini.car.jumpcar = settings.car.jumpcar[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Анти карскилл', settings.car.anticarskill) then
                    if settings.cfg.autosave[0] then
                        ini.car.anticarskill = settings.car.anticarskill[0]
                        save()
                    end
                end
                if imgui.Button('Флипнуть') then
            ...
            elseif subtab_3 == 2 then
                if imgui.ToggleButton('Attach Trailer', settings.car.atractive) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.SliderInt('Радиус', settings.car.atrradius, 1, 500) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_3 == 3 then
                if imgui.ToggleButton('SpeedHаck', settings.car.speedhack) then
                    if settings.cfg.autosave[0] then
                        ini.car.speedhack = settings.car.speedhack[0]
                        save()
                    end
                end
                if imgui.Button('Настройки спидхака') then
                    imgui.OpenPopup('Настройки спидхака')
                end
                if imgui.BeginPopupModal('Настройки спидхака', _, imgui.WindowFlags.AlwaysAutoResize) then
                    if imgui.SliderFloat('Сила ускорения', settings.car.slider_mult, 0.001, 100.0) then
			            if settings.cfg.autosave[0] then
			                ini.car.mult = settings.car.slider_mult[0]
			                save()
			            end
		            end
	        	    if imgui.SliderFloat('Максимальная скорость', settings.car.slider_limit, 0.01, 1000.0) then
			            if settings.cfg.autosave[0] then
			                ini.car.limit = settings.car.slider_limit[0]
			                save()
			            end
		            end
		            if imgui.SliderFloat('Шаг (мс)', settings.car.slider_timestep, 0.0, 1.0) then
			            if settings.cfg.autosave[0] then  
			                ini.car.timestep = settings.car.slider_timestep[0]
			                save()
			            end
		            end
		            if imgui.ToggleButton('Безопасная скорость поезда', settings.car.safe_train_speed) then
			            if settings.cfg.autosave[0] then 
        ...
        elseif tab == 4 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button("ESP") then
                subtab_4 = 1
            end
            imgui.SameLine()
            if imgui.Button("Настройки ESP") then
                subtab_4 = 2
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_4 == 1 then
                if imgui.ToggleButton('Включить рисовку', settings.ESP.drawing) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.drawing = settings.ESP.drawing[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Коробки', settings.ESP.enabled_boxes) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_boxes = settings.ESP.enabled_boxes[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Ники', settings.ESP.enabled_nicks) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_nicks = settings.ESP.enabled_nicks[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Скелеты', settings.ESP.enabled_bones) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_bones = settings.ESP.enabled_bones[0]
                        ini.car.speedhack = settings.car.speedhack[0]
                    end
                end
                if imgui.ToggleButton('Линии', settings.ESP.enabled_lines) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_lines = settings.ESP.enabled_lines[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Здоровье', settings.ESP.enabled_health) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_health = settings.ESP.enabled_health[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Броня', settings.ESP.enabled_armor) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_4 == 2 then
                if imgui.SliderInt('Позиция линии', settings.ESP.lineposition, 0, 1) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.lineposition = settings.ESP.lineposition[0]
                        save()
                    end
                end
                if imgui.SliderInt('Толщина линии', settings.ESP.linethinkness, 1, 4) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.linethinkness = settings.ESP.linethinkness[0]
                        save()
                    end
                end
                if imgui.SliderInt('Толщина скелетов', settings.ESP.skeletonthinkness, 1, 4) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.skeletonthinkness = settings.ESP.skeletonthinkness[0]
                        save()
                    end
                end
                if imgui.SliderInt('Толщина коробок', settings.ESP.boxthinkness, 1, 4) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.boxthinkness = settings.ESP.boxthinkness[0]
                        save()
                    end
                end
                if imgui.SliderFloat('Размер никнеймов', settings.ESP.nicknamesize, 1, 10) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.ESP.enabled_boxes[0] then
                    if imgui.ToggleButton('Градиентная заполненность', settings.ESP.enabled_fillgradient) then
                        if settings.cfg.autosave[0] then
                            ini.ESP.enabled_fillgradient = settings.ESP.enabled_fillgradient[0]
                            save()
                        end
                    end
                    if imgui.SliderFloat('Заполненность коробок', settings.ESP.fillboxvalue, 0.0, 1.0) then
                        if settings.cfg.autosave[0] then
        ...
        elseif tab == 5 then
            ...
            imgui.SameLine()
            if imgui.Button("ТП к стиралкам") then
                subtab_5 = 2
            end
            imgui.SameLine()
            if imgui.Button("Утилиты") then
                subtab_5 = 3
            end
            imgui.SameLine()
            if imgui.Button("Грузчик") then
                subtab_5 = 4
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_5 == 1 then
                if imgui.Button('Удалить клетки') then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                ...
                imgui.SameLine()
                if imgui.Button('Удалить дверь') then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                ...
                imgui.SameLine()
                if imgui.Button('Удалить заборы') then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                ...
                if imgui.ToggleButton('Aвтoматич.', settings.tsr.autormcell) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Aвтоматич.', settings.tsr.autormdoors) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                if imgui.ToggleButton('Автoматич.', settings.tsr.autormfence) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_5 == 2 then
                ...
                if imgui.Button('Грязная одежда') then
                    setCharCoordinates(PLAYER_PED, 1396.1380, 1354.9414, 10)
                end
                imgui.SameLine()
                if imgui.Button('Чистая одежда') then
                    setCharCoordinates(PLAYER_PED, 1397.8023, 1357.9216, 10.9390)
                end
            elseif subtab_5 == 3 then
                if imgui.ToggleButton('Вх на мусор', settings.render.musortsr) then
                    if settings.cfg.autosave[0] then
                        ini.render.musortsr = settings.render.musortsr[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Бот коробки RAGE', tsrragebot) then
                    botstep = 0
                end
                if imgui.SliderInt('Задержка', settings.tsr.tsrbotwait, 100, 5000) then
                    if settings.cfg.autosave[0] then
        ...
        elseif tab == 6 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button("Кнопочки") then
                subtab_6 = 1
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_6 == 1 then
                if imgui.Button(fa.COMMENT..'  Очистить чат', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    for i = 1, 90 do
                        sampAddChatMessage("", -1)
                    end
                end
                imgui.SameLine()
                if imgui.Button(fa.SKULL_CROSSBONES..'  Умереть', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    setCharHealth(PLAYER_PED, 0)
                end
                imgui.SameLine()
                if imgui.Button(fa.HOUSE..'  Спавн', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    sendSpawn()
                end
                imgui.SameLine()
                if imgui.Button(fa.PERSON_WALKING..'  Вкл. прыжок', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    lua_thread.create(function()
                ...
                imgui.SameLine()
                if imgui.Button(fa.LOCATION_DOT..'  ТП метка', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    result, x, y, z = getTargetBlipCoordinatesFixed()
                    if result then setCharCoordinates(PLAYER_PED, x, y, z) end
                end
                if imgui.Button(fa.WIFI..'  Реконнект v2', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    notf4('Реконнектимся...')
                    local bs = raknetNewBitStream()
                    raknetBitStreamWriteInt8(bs, sf.PACKET_DISCONNECTION_NOTIFICATION)
                    raknetSendBitStreamEx(bs, sf.SYSTEM_PRIORITY, sf.RELIABLE, 0)
                ...
                if imgui.Button(fa.CARET_UP..'  Slap up', imgui.ImVec2(125 * MONET_DPI_SCALE, 31 * MONET_DPI_SCALE)) then
                    local x, y, z = getCharCoordinates(PLAYER_PED)
                    setCharCoordinates(PLAYER_PED, x, y, z+5)
                end
                if imgui.Button(fa.CARET_DOWN..'  Slap down', imgui.ImVec2(125 * MONET_DPI_SCALE, 31 * MONET_DPI_SCALE)) then
                    local x, y, z = getCharCoordinates(PLAYER_PED)
                    setCharCoordinates(PLAYER_PED, x, y, z-5)
                end
            end
        elseif tab == 7 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button("Открытие меню") then
                subtab_7 = 1
            end
            imgui.SameLine()
            if imgui.Button("Меню") then
                subtab_7 = 2
            end
            imgui.SameLine()
            ...
            imgui.SameLine()
            if imgui.Button("Скрипт") then
                subtab_7 = 4
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_7 == 1 then
                if imgui.ToggleButton('Полоска снизу экрана', settings.menu.openbutton) then
                    if settings.cfg.autosave[0] then
                        ini.menu.openbutton = settings.menu.openbutton[0]
                        save()
                    end
                end
                if imgui.ToggleButton('Кнопка "Menu"', settings.menu.openbutton2) then
                    if settings.cfg.autosave[0] then
                        ini.menu.openbutton2 = settings.menu.openbutton2[0]
                        save()
                    end
                end
		        if imgui.SliderInt('Ширина кнопки меню', settings.menu.slideropenbuttonwidth, 30, 150) then
                    if settings.cfg.autosave[0] then
                        ini.menu.slideropenbuttonwidth = settings.menu.slideropenbuttonwidth[0]
                        save()
                    end
                end
		        if imgui.SliderInt('Высота кнопки меню', settings.menu.slideropenbuttonheight, 30, 150) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_7 == 2 then
                if imgui.Button('Настройки меню') then
		            menusettings[0] = not menusettings[0]
		        end
		        if imgui.ToggleButton('Включить спецэффекты меню', settings.menu.draweffects) then
		            if settings.cfg.autosave[0] then
		                ini.menu.draweffects = settings.menu.draweffects[0]
		                save()
		            end
		        end
                if imgui.ToggleButton('Ватермарка', settings.menu.watermark) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_7 == 3 then
                if imgui.ToggleButton('Отображать подсказки', settings.menu.showinfo) then
                    if settings.cfg.autosave[0] then
                        ini.menu.showinfo = settings.menu.showinfo[0]
                        save()
                    end
                end
		        if imgui.Button('Показать кнопку взаимодействия') then
		        ...
		        imgui.SameLine()
		        if imgui.Button('Команды скрипта') then
		            imgui.OpenPopup('Команды скрипта')
		        end
                if imgui.BeginPopupModal('Команды скрипта', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.Text('/miku - основное меню скрипта')
                    imgui.Text('/tpc - телепорт метка (для нубо рп, моментальный)')
                    imgui.Text('/jump - включить прыжок (баг, когда упал и перс не встает)')
                    imgui.Text('/rc - реконнект')
                    imgui.Text('/mikureload - перезагрузка скрипта')
                    if imgui.Button(' Закрыть') then
                        imgui.CloseCurrentPopup()
                    end
                    imgui.EndPopup()
            ...
            elseif subtab_7 == 4 then
                ...
                if imgui.Button('Показать mimgui demo') then
		            custommimguiStyle[0] = not custommimguiStyle[0]
		        end
                if imgui.Button('Обновиться на стабильную версию') then
                    updateScript(lmUrl, lmPath)
                end
                if imgui.Button('Обновиться на Бета-версию') then
                ...
                if imgui.IsItemHovered() then
                    ...
                    imgui.TextUnformatted('Выключить скрипт')
                ...
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.PushTextWrapPos(450)
                    imgui.TextUnformatted('Перезагрузить скрипт')
                    imgui.PopTextWrapPos()
                    imgui.EndTooltip()
                end
                imgui.SameLine()
                if imgui.Button(fa.FLOPPY_DISK, imgui.ImVec2(40 * MONET_DPI_SCALE, 40 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup('Сохранение')
                end
                if imgui.BeginPopupModal('Сохранение', _, imgui.WindowFlags.NoResize) then
                    local pSize = imgui.ImVec2(300, 500)
                    imgui.SetWindowSizeVec2(imgui.ImVec2(230 * MONET_DPI_SCALE, 150 * MONET_DPI_SCALE))
                    if imgui.Button('Сохранить настройки', imgui.ImVec2(205 * MONET_DPI_SCALE, 40)) then
                        ini.silent.salo = settings.silent.salo[0]
                        ...
                        ini.battery.lowBatteryLevel = settings.battery.lowBatteryLevel[0]
                        save()
                        notf1('Настройки сохранены!')
                        imgui.CloseCurrentPopup()
                    end
                    if imgui.ToggleButton('Автосохранение', settings.cfg.autosave) then
                        ini.cfg.autosave = settings.cfg.autosave[0]
                        save()
                    end
                    if imgui.Button('Закрыть', imgui.ImVec2(205 * MONET_DPI_SCALE, 40)) then
                        imgui.CloseCurrentPopup()
                    end
                    imgui.EndPopup()
                end
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.PushTextWrapPos(450)
                    imgui.TextUnformatted('Сохранить настройки')
                    imgui.PopTextWrapPos()
                    imgui.EndTooltip()
                end
                imgui.SameLine()
                ...
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.PushTextWrapPos(450)
                    imgui.TextUnformatted('Удалить скрипт')
                    imgui.PopTextWrapPos()
                    imgui.EndTooltip()
                end
                if settings.menu.showinfo[0] then  
                    imgui.SameLine()
                    imgui.TextDisabled('(?)', imgui.ImVec2(30 * MONET_DPI_SCALE, 30 * MONET_DPI_SCALE))
                    if imgui.IsItemHovered() then
                        imgui.BeginTooltip()
                        imgui.PushTextWrapPos(450)
                        imgui.TextUnformatted('Зажмите кнопку пальцем, чтоб посмотреть подсказку')
                        imgui.PopTextWrapPos()
                        imgui.EndTooltip()
                    end
                end
                if deletescript then
                    imgui.Text('Вы уверены?')
...
function main()
    ...
    check_update()
    antifall()
    jniUtil.Toast("Miku Reborn загружен!", jniUtil.ToastFlag.LENGTH_SHORT):show()
        ...
        if settings.battery.notifyLowCharge[0] and success or settings.battery.notifyLowCharge[0] and success2 then
            ...
            if getBatteryLevel <= settings.battery.lowBatteryLevel[0] then
                if not settings.battery.stopMessage then
                    ...
                    jniUtil.Toast("[Miku] Низкий заряд батареи!!", jniUtil.ToastFlag.LENGTH_SHORT):show()
                end
            else
...
function events.onServerMessage(clr, txt) 
	if armorbotstate then
		...
		if string.find(txt, 'ответил вам') or string.find(txt, 'телепортированы') then	
		    notf2('Вам написал администратор, закругляемся...')
			lua_thread.create(function()
...
-- message form
function notf1(text)
    Notifications.Show('Miku Project Reborn:\n'..text, Notifications.TYPE.OK)
end
...
function check_update()
    notf4('Проверка наличия обновлений...')
    ...
    if response and response ~= currentVersion then
        notf4("Найдено новое обновление! Вывожу окно для загрузки...")
        found_update[0] = not found_update[0]
    else
        notf1("У вас актуальная версия скрипта.")
    end
end
...
function updateScript(scriptUrl, scriptPath)
    ...
    if response and response ~= currentVersion then
        ...
        if success then
            notf1("Скрипт успешно обновлен. Перезагрузка..")
            thisScript():reload()
        else
            notf3("Неизвестная ошибка, не удалось обновить скрипт.")
        end
    else
        notf1("Скрипт уже является последней версией.")
    end
end
