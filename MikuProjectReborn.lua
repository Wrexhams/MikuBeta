...
local derevovish = {
	[729] = u8'Дерево высшего качества',
}

local apple = {
	[19576] = u8'Яблоко (или слива)',
    [895] = u8'Яблочное дерево'
}
        ...
        if settings.render.ruda[0] then
            for id = 0, 2048 do
                if sampIs3dTextDefined(id) then
                    ...
                    if text:find(u8"Месторождение ресурсов") then
...
function events.onServerMessage(clr, txt) 
	if armorbotstate then
		if string.find(txt, u8'Вы сделали бракованную деталь') then
			lua_thread.create(function()
		...
		if string.find(txt, u8'ответил вам') or string.find(txt, u8'телепортированы') then	
...
function events.onShowDialog(dlgid, stl, tlt, b1, b2, text)
	if armorbotstate and string.find(text, u8'Администратор') or string.find(text, u8'телепортированы') then
		notf2(u8'Вам написал администратор, закругляемся...')
		lua_thread.create(function()
			wait(math.random(1470, 1955))
			point = 0
...
function getBatteryPercentage()
    ...
    if batteryManager == nil then
        sampAddChatMessage(u8"[Miku] Ошибка: не удалось получить сервис BatteryManager", -1)
        sampAddChatMessage(u8"[Miku] Процент заряда в Watermark не будет отображаться корректно", -1)
        return -1
    end
    ...
    if ok then
        ...
        sampAddChatMessage(u8"[Miku] Ошибка вызова getIntProperty(BATTERY_PROPERTY_CAPACITY): " .. tostring(result), -1)
        percentage = -1
    end
